...
def categorize(title):
    t = title.lower()
    if any(k in t for k in ["high-speed","high speed","toc do cao","speed rail","hsr"]):
        return "High-Speed Rail"
    if any(k in t for k in ["metro","mrt","ben thanh","suoi tien","nhon","cat linh"]):
        return "Metro"
    if any(k in t for k in ["railway","railroad","train","duong sat","ga "," ga"]):
        return "Railway"
    return "Transport"
...
def get_news():
    ...
    vi.sort(key=sort_key, reverse=True)
    en.sort(key=sort_key, reverse=True)
    results = vi + en
    for a in results:
        a.pop("dt", None)
    return results
